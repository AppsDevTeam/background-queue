...
`background-queue:monitor` Jednorázově spočítá joby ve stavech `TEMPORARILY_FAILED` a `PERMANENTLY_FAILED` — jediných dvou, ve kterých se joby drží dlouhodobě (ostatní stavy mají aktivní pojistku, viz 2.4 a 2.5). Počty vypíše na výstup a je-li co hlásit, pošle report i do loggeru: s úrovní `critical`, obsahuje-li `PERMANENTLY_FAILED` joby (ty se bez ručního zásahu už nikdy nespustí), jinak `warning`. Ideální spouštět cronem jednou denně o půlnoci (`0 0 * * *`).
