...
#[AsCommand(name: 'background-queue:monitor', description: 'Report the number of failed jobs.')]
class MonitorCommand extends Command
{
	...
	{
		$report = $this->backgroundQueue->reportFailedJobs();
		...
		foreach ($report as $_state => $_data) {
			$output->writeln(sprintf(
				...
				$_state,
				$_data['count'],
				$_data['oldestCreatedAt'] ? ' (oldest ' . $_data['oldestCreatedAt'] . ')' : ''
			));
		}
